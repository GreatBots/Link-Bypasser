...
def loopthread(message, otherss=False):
    ...
    if bypasser.ispresent(bypasser.ddl.ddllist, urls[0]):
        ...
        if "https://olamovies" in urls[0] or "https://psa.wf/" in urls[0]:
            msg = app.send_message(message.chat.id, "⏳ __this might take some time...__",
    ...
    try:
        ...
        tmsgid = message.id
        for ele in final:
            tmsg = app.send_message(message.chat.id, f'__{ele}__', reply_to_message_id=tmsgid,
...
def send_start(client: pyrogram.client.Client, message: pyrogram.types.messages_and_media.message.Message):
    app.send_message(message.chat.id,
                     f"👋🏻 Hi **{message.from_user.mention}**, __i am Link Bypasser Bot,\n\njust send me any supported links and i will you get you results.\n\nCheckout /help to Read More__",
                     reply_markup=InlineKeyboardMarkup([
                         [InlineKeyboardButton("📣 Updates", url="https://better_botz.t.me")],
                         [InlineKeyboardButton("Dev", url="https://anocy.t.me")]]),
                     reply_to_message_id=message.id)
